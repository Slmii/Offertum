 ...
 * (no persisted table) for an authenticated JSON read + a public token-auth iCal feed. The
 * whole feature is subscription-gated: the authenticated controller via CalendarEntitlementGuard,
 * the public feed via CalendarService.renderFeed's own entitlement check.
 */
@Module({
	controllers: [CalendarController, CalendarIcalController],
	providers: [CalendarService, CalendarRepository, CalendarEntitlementGuard]
})
