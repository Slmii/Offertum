...
function scopeFromRequest(request: Request): { organizationId: string; userId: string } {
...
export class GmailController {
	...
	async callback(
		...
		await this.accounts.upsertGmail({
			organizationId,
			userId,
	...
	async status(@Req() request: Request): Promise<GmailStatusResponseDto> {
		...
		const account = await this.accounts.findGmailAccount(scope);
		return {
	...
	async disconnect(@Req() request: Request): Promise<GmailDisconnectResponseDto> {
		const scope = scopeFromRequest(request);
		await this.accounts.disconnectGmail(scope);
		return { ok: true };
	}
}
