...
export class DigestService {
	...
	async runDailyDigest(now: Date = new Date()): Promise<{ orgs: number; recipients: number; skippedDuplicate: number }> {
		...
		for (const org of orgs) {
			...
			const expiringItems = await this.digestRepository.findExpiringCallouts(org.id);
