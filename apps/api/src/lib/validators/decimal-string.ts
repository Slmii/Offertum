...
/** Money: Prisma `Decimal(10, 2)` — up to 8 integer digits + up to 2 decimals. A
 * leading `-` is allowed because engine-produced discount ("Korting") lines carry a
 * negative `unitPriceEur`, and a draft holding one must survive a regenerate/replace. */
export const MONEY_DECIMAL_PATTERN = /^-?\d{1,8}(\.\d{1,2})?$/;
export const MONEY_DECIMAL_MESSAGE =
	'must be a decimal with up to 8 integer digits and up to 2 decimals (optional leading minus)';
