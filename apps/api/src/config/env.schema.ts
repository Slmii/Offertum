...
export const envSchema = z
	.object({
		...
		// Swagger docs basic-auth credentials. Both optional in dev (when unset the
		// `/docs` UI + `/docs/openapi.json` raw spec stay open + a warn log fires at boot).
		// Required in production — the cross-field validation below rejects boot when
		// `NODE_ENV=production` and either is missing, so we never accidentally ship the
