...
- **Interactive non-button elements → MUI `ButtonBase`, never `Box component='button'`.** Any clickable element that isn't a real `<Button>` (custom triggers, clickable cards/rows/tiles) uses `ButtonBase` (focus ring, keyboard activation, ripple-off if needed) instead of a hand-rolled `Box component='button'`.
- **Icon-only buttons → MUI `IconButton`, never `Box component='button'`.** A button whose content is just an icon (delete, close, dropdown caret, etc.) uses `IconButton` (correct sizing, hover/focus states, `aria-label`).
- **Typography: prefer inline props over `sx`.** On the `Text` components (`H1`/`H2`/`H3`/`Body`/`BodySmall`/…) use the dedicated props where they exist — `fontWeight`, `color`, etc. — e.g. `<H3 fontWeight='bold'>` not `<H3 sx={{ fontWeight: 'bold' }}>`. Reserve `sx` for properties without a prop.
- **Mutation errors → toast, never a banner.** When a mutation (`useMutation`/`.mutate`) fails, surface it via `useToast().error(title, message)` in the mutation's `onError` — never render a `Banner`/`BannerStack` off `mutation.isError`. Banners are for persistent, page-level notices (validation, entitlement, informational state); transient action failures are toasts. (Query/loader errors still use `SectionError`/`errorComponent` — this rule is specifically about mutation failures.)
- **Plurals → `pluralize(count, singular, plural)`** from `@offertum/shared`. Never inline `count === 1 ? 'regel' : 'regels'` (or suffix forms like `regel${n===1?'':'s'}`). Caller renders the count: `${n} ${pluralize(n, 'regel', 'regels')}`. Phrases work too: `pluralize(n, 'regel heeft', 'regels hebben')`. (In-house, not the English-only npm package — it can't do Dutch irregulars or verb phrases.)
- **Dialogs render unconditionally with an `open`/`isOpen` prop — never `{cond && <Dialog>}`.** Conditionally mounting a dialog unmounts it the instant it closes, killing the MUI close transition. Always render it and drive visibility with the boolean: `<Dialog open={isOpen} …>`. A dialog-wrapper component (e.g. `CatalogItemDialog`, `DismissDialog`) exposes an `isOpen` prop and forwards `open={isOpen}` to the base `Dialog`; callers render it always (`<DismissDialog isOpen={dismissOpen} …>`). An outer guard is fine only for a genuine *data precondition* that doesn't change on open/close (e.g. `{latest && <RegenModal isOpen={open} draft={latest} …>}`) — never for the open state itself. When a dialog stays mounted, re-seed any state derived from a prop when that prop changes (adjust-state-during-render, not an effect).
