...
- `apply-review.yml` is human-gated (only runs on the `apply-ai-review` label) and pushes with the default `GITHUB_TOKEN`, so its commit does not re-trigger `pr-review.yml` — no review↔apply ping-pong. It only applies high-confidence feedback and reports what it skipped.
